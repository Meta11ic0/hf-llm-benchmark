...
>
> **本文档定位**：以 9 个任务为主干，每节含「要做什么 → 步骤 → 踩坑 → 验收」。
>
> **关于代码**：每个需要写脚本的任务都给了 API 清单和设计要点。你可以手写（都是 30 行左右的脚本），也可以让 AI 生成——两者都行。**唯一不能跳过的**：每行代码你都要能解释它在做什么。验收标准就是你的自检清单。
>
> **关于 CLAUDE.md**：项目根目录的 `CLAUDE.md` 是配合本项目使用的 AI 助教设定，融入了作者本人的学习偏好（追问检验、短周期正反馈等）。Clone 后可根据自己的习惯微调——如果你不需要追问环节或者想调整解释深度，直接改那个文件就行。
...
| **EOS** | End Of Sequence，模型生成的特殊 token，表示"我说完了" | 字符串的终止符 |
| **自回归** | 一次生成一个 token，每个新 token 依赖之前所有已生成的 token——不能一口气出全部结果 | 像写字：每一笔都基于前面写过的内容，不可能同时写完一整篇 |
| **tensor** | PyTorch 里的多维数组。2D = 矩阵，1D = 向量 | `float matrix[1024][1024]` 就是一个 2D tensor |
| **loss / 损失** | 模型输出与正确答案之间的差距，训练的目标是让这个值尽量小 | 考试得分与满分的差距 |
| **dtype** | 数组中每个数的存储格式（fp32=4字节，bf16=2字节，int8=1字节） | 选 `float`（4B）还是 `int8`（1B）存数据 |

**项目结构**（你需要写脚本的任务，参考实现放在 `reference/`）：
...
```bash
# 假设已在项目根目录（hf-llm-benchmark/）下
python3 -m venv venv
source venv/bin/activate
```

`venv` 按项目隔离 Python 包——不同项目可以用不同版本的 `torch`，互不冲突。类似 Node.js 的 `node_modules` 或 Docker 的容器隔离。
...
创建 `minimal_infer.py`（约 25 行），跑一条 prompt，打印 chat_template / input_ids / 模型回复。这是所有后续任务的代码地基。手写或 AI 生成均可——写完对照 `reference/minimal_infer.py`，确保每行你都能解释。
...
```
AutoTokenizer.from_pretrained(MODEL_ID)       → tokenizer 对象
AutoModelForCausalLM.from_pretrained(...)     → model 对象（dtype=float32）
model.eval()                                   → 推理模式
tokenizer.apply_chat_template(messages, ...)  → 格式化字符串（add_generation_prompt=True）
tokenizer(prompt, return_tensors="pt")        → input_ids
model.generate(**inputs, max_new_tokens=N)    → output_ids（用 torch.no_grad() 包裹）
tokenizer.decode(new_ids, ...)                → 文本（先切片去掉 prompt 部分）
```

**关键点**：
- `add_generation_prompt=True`：漏掉会输出空白或重复 prompt
- `output_ids` = 输入 token + 新生成 token，decode 前要切片取新 token
- `do_sample=False`：每步贪心选最高分，输出确定可复现
- `torch.no_grad()`：推理不需要梯度，省内存

写完后对照 `reference/minimal_infer.py`。
...
| 文件 | 大小 | 是什么 | 类比 |
|------|------|--------|------|
| `model.safetensors` | ~1.4 GB | 全部权重（~6 亿浮点数） | 程序的 `.so` / `.dll`——可执行数据 |
| `tokenizer.json` | ~11 MB | 151936 个 token 的映射表 | 编码表，字 → 数字 |
| `config.json` | ~1.5 KB | 架构参数（层数、维度、词表大小） | 配置文件，类似 `package.json` |
...
```python
# 交互式 Python，或者存为 explore_tokenizer.py
...
**自己试试**：在 1.3 的 tokenizer 交互实验中，把 `<|im_start|>` 作为文本输入 `tok.encode()`，观察它是否被识别为单个特殊 token 而非多个普通字符。
...
```
messages ──chat_template──▶ prompt 字符串 ──tokenize──▶ input_ids ──generate──▶ output_ids ──decode──▶ 回复文本
```
...
创建 `gen_params.py`，系统对比 temperature / top_p / max_new_tokens 对速度与输出的影响。脚本内容：加载模型 → 对固定 prompt 分别测试不同参数 → 打印每组耗时和生成 token 数。手写或 AI 生成均可——参考 `reference/gen_params.py`。
...
创建 `benchmark.py`：3 prompt × 3 次，输出 avg / P50 / P99 / tokens/s；含预热（丢弃首次）。手写或 AI 生成均可——参考 `reference/benchmark.py`。
...
Meta 的 gated model 审核是自动化的，中国 IP + 中国地址可能在系统层面被拒（Llama 3.1 起生效）。该限制源自美国出口管制法规（BIS 将 AI 模型权重纳入出口管制编码 ECCN 4E091），非 HF 平台本身。如果遇到，用 config 对比代替实测即可。
...
创建 `quantization.py`：加载 Qwen3-0.6B（dtype=float32），用 `sum(p.numel() for p in m.parameters())` 获取参数量，分别乘 4（FP32）和 1（INT8）得到理论内存占用。额外看下磁盘缓存中 safetensors 的实际大小做对比。手写或 AI 生成均可——参考 `reference/quantization.py`。
...
```
磁盘（bf16+部分fp32）         内存（fp32）            内存（int8，量化后）
   safetensors 1.5 GB  ──加载──▶  2.4 GB  ──量化──▶  0.6 GB
   官方发布，你改不了               dtype 参数控制            CPU 不可用，理论推算
```
...
用代码的角度看：**量化就是把 4 字节浮点数转成 1 字节整数**，用精度换空间。

```
float  original = 1.234;         // 全精度，4 字节
int8   quantized = original * 127; // 映射到 [-127, 127]，1 字节
float  restored = quantized / 127; // ≈ 1.22... 有误差了
```
...
2. 看一眼 `CLAUDE.md`——那是配合本项目用的 AI 助教设定。如果其中的追问环节、解释风格等不符合你的习惯，直接修改
3. 整理想法与笔记，能用 5 分钟口述：prompt → token → generate → decode → benchmark
4. `git add` / `git commit` / `git push`，GitHub 公开仓库
